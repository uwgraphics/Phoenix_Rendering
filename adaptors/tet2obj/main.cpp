...
Write_Obj_File(const TETRAHEDRALIZED_VOLUME<T>& tet_volume,const std::string& filename)
{
	...
	int index_min = 1;
	int index_max = tet_volume.particles.array_collection->Size();

		//LOG::cout << "Min Index: " << index_min << std::endl;
		//LOG::cout << "Max Index: " << index_max << std::endl;
	

    for(int p=1;p<=tet_volume.particles.array_collection->Size();p++)
		{
			output<<"v";
			for(int i=1;i<=d1;i++)
				output<<" "<<tet_volume.particles.X(p)(i);
			if(d1==2) output<<" 0.0";
			output<<std::endl;
		}

    for(int e=1;e<=tet_volume.mesh.elements.m;e++)
		{
			output<<"f";
			for(int i=1;i<=d2;i++)
				{
					
						//PHYSBAM_ASSERT( tet_volume.mesh.elements(e)(i) >= index_min );
						//PHYSBAM_ASSERT( tet_volume.mesh.elements(e)(i) <= index_max );
					output<<" "<<tet_volume.mesh.elements(e)(i);
				}
			output<<std::endl;
		}
...
int main(int argc,char* argv[])
{
	...
	FILE_UTILITIES::Read_From_File(stream_type,input_filename,tet_volume);

	Write_Obj_File(tet_volume,output_filename);
