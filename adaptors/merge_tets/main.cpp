...
int main(int argc,char* argv[])
{
    ...
    TETRAHEDRALIZED_VOLUME<T>& tetrahedralized_volumeA=*TETRAHEDRALIZED_VOLUME<T>::Create();
	FILE_UTILITIES::Read_From_File(stream_type,inputA_filename,tetrahedralized_volumeA);

    TETRAHEDRALIZED_VOLUME<T>& tetrahedralized_volumeB=*TETRAHEDRALIZED_VOLUME<T>::Create();
	FILE_UTILITIES::Read_From_File(stream_type,inputB_filename,tetrahedralized_volumeB);

	
	ARRAY<TETRAHEDRALIZED_VOLUME<T> *> surf_array;
	surf_array.Resize(2);
	surf_array(1) = &tetrahedralized_volumeA;
	surf_array(2) = &tetrahedralized_volumeB;

	ARRAY<FRAME<TV> > frames;
	frames.Resize( 2 );

    TETRAHEDRALIZED_VOLUME<T>& tetrahedralized_volumeC=*TETRAHEDRALIZED_VOLUME<T>::Union_Mesh_Objects_Relatively(surf_array,frames);
	

	FILE_UTILITIES::Write_To_File(stream_type,output_filename,tetrahedralized_volumeC);

	delete &tetrahedralized_volumeA;
	delete &tetrahedralized_volumeB;
	delete &tetrahedralized_volumeC;
